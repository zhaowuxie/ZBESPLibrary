...
!function () {
    var Helper = {
        ...
        attachEvent: function attachEvent(el, handler, capture) {
            ...
            el.addEventListener(event.move, handler.move, capture);
        ...
        detachEvent: function detachEvent(el, handler) {
            ...
            el.addEventListener(event.move, handler.move);
    ...
    var newHandler = function newHandler(type) {
        return {
            ...
            move: function (e) {
                ...
                var h = (currentX - start.X) / ratio;
                var v = (currentY - start.Y) / ratio;
                var absh = Math.abs(h);
                var absv = Math.abs(v);
                var move = Math.sqrt(Math.pow(h, 2) + Math.pow(v, 2));
                if (move > 20) {
